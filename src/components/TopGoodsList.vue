<template>
  <div>
       <yd-navbar title="头筹订单详情" height='.8rem' color="#f2f2f2" bgcolor="#ff5f17">
        ...
        <yd-tab-panel v-for="item in items" :label="item.label" :key="item.id">
                <yd-preview :buttons="btns" v-for="itemt in GoodsHtml" :key="itemt.id">
                        <yd-preview-header>
                        ...
                        <yd-preview-item>
                            <div slot="left">幸运号码:{{itemt.LuckerNumber}}</div>
                            <div slot="right">{{itemt.IsWinner==1?'中奖':'未中奖'}}</div>
                        </yd-preview-item>
              </yd-preview>
        </yd-tab-panel>
    </yd-tab>
